...
if exist "%~dp0venv\Scripts\python.exe" (
    ...
    echo        [Info] Found virtual environment, using GPU accelerated Python
)
...
if exist "build_desktop\pdp_desktop.exe" (
    echo Starting: build_desktop\pdp_desktop.exe
    start "" "build_desktop\pdp_desktop.exe"
    exit /b 0
)

if exist "build\pdp_desktop.exe" (
    echo Starting fallback: build\pdp_desktop.exe
    start "" "build\pdp_desktop.exe"
    exit /b 0
)

if exist "pdp_desktop.exe" (
    echo Starting fallback: pdp_desktop.exe
